...
impl AuraWallet {
    ...
    pub fn create_presentation(
        ...
        let presentation_generator = PresentationGenerator::new(self.vc_store.clone(), self.did_manager.clone());
        presentation_generator.create_presentation(
            holder_did,
            credential_ids,
            challenge,
            domain,
...
mod tests {
    use super::*;
    use aura_common::{CredentialIssuer, CredentialSubject, VerifiableCredential, Timestamp};
    ...
    fn test_find_credentials_by_type() {
        ...
        let mut cred1 = create_test_credential("1".to_string(), issuer_did.clone(), holder_did.clone());
        cred1.credential_type = vec!["VerifiableCredential".to_string(), "Diploma".to_string()];
        
        let mut cred2 = create_test_credential("2".to_string(), issuer_did.clone(), holder_did.clone());
    ...
    fn test_verify_credential() {
        ...
        // Create credential
        let mut credential = create_test_credential("cred123".to_string(), issuer_did.clone(), subject_did);
        
        // Sign the credential
        let signature = aura_crypto::signing::sign_json(issuer_keypair.private_key(), &credential).unwrap();
        ...
        // Verify
        let valid = wallet.verify_credential(&credential, issuer_keypair.public_key()).unwrap();
        assert!(valid);
    }
    ...
    fn test_create_selective_presentation() {
        ...
        let presentation = wallet.create_selective_presentation(
            &holder_did,
            "cred1".to_string(),
            vec!["name".to_string()],
            None,
    ...
    fn test_verify_presentation() {
        ...
        // Verify
        let valid = wallet.verify_presentation(
            &presentation,
    ...
    fn test_export_import_wallet() {
        ...
        wallet1.store_credential(cred1, vec!["tag1".to_string()]).unwrap();
        wallet1.store_credential(cred2, vec!["tag2".to_string()]).unwrap();
    ...
    fn test_multiple_operations() {
        ...
        for (i, did) in dids.iter().enumerate() {
            ...
            wallet.store_credential(credential, vec![format!("tag{}", i)]).unwrap();
        }
