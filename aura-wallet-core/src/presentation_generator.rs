...
impl PresentationGenerator {
    ...
    pub fn create_presentation(
        ...
        for id in credential_ids {
            let stored_cred = self.vc_store.get_credential(&id)?
        ...
        // Add proof
        let proof = self.create_presentation_proof(
            holder_did,
            &presentation,
            challenge,
            domain,
    ...
    pub fn create_selective_presentation(
        ...
        // Get the credential
        let stored_cred = self.vc_store.get_credential(&credential_id)?
            .ok_or_else(|| AuraError::NotFound(format!("Credential {} not found", credential_id)))?;
