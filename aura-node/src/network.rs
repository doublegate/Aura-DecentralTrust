...
pub struct NetworkTopics {
    pub blocks: Topic,
    pub transactions: Topic,
    pub did_updates: Topic,
}

impl NetworkTopics {
    fn new() -> Self {
        Self {
            blocks: Topic::new("aura/blocks/1.0.0"),
            transactions: Topic::new("aura/transactions/1.0.0"),
            did_updates: Topic::new("aura/did-updates/1.0.0"),
        }
    }
}
...
impl NetworkManager {
    pub async fn new(config: NetworkConfig) -> anyhow::Result<Self> {
        ...
        for bootstrap in &config.bootstrap_nodes {
            if let Ok(multiaddr) = bootstrap.parse() {
        ...
        match event {
            AuraNetworkBehaviourEvent::Gossipsub(gossipsub::Event::Message {
                ...
                match bincode::deserialize::<NetworkMessage>(&message.data) {
                    Ok(network_msg) => {
                        if topic == self.topics.blocks {
                            self.handle_new_block(network_msg).await;
                        } else if topic == self.topics.transactions {
                            self.handle_new_transaction(network_msg).await;
                        } else if topic == self.topics.did_updates {
            ...
            AuraNetworkBehaviourEvent::Identify(identify::Event::Received { peer_id, info }) => {
...
use libp2p::swarm::NetworkBehaviour as NetworkBehaviourTrait;