...
static DID_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^did:aura:[a-zA-Z0-9\-_]+$").unwrap()
});
...
pub fn validate_schema_id(schema_id: &str) -> Result<()> {
    if !SCHEMA_ID_REGEX.is_match(schema_id) {
        return Err(AuraError::Validation("Invalid schema ID format".to_string()));
    }
    ...
    if let Some(host) = parsed.host_str() {
        ...
        if host.starts_with("10.") || 
           ...
           host.starts_with("192.168.") {
            return Err(AuraError::Validation("Private IP addresses not allowed".to_string()));
        }
    }
...
pub fn validate_transaction_size(data: &[u8]) -> Result<()> {
    if data.len() > MAX_TRANSACTION_SIZE {
        return Err(AuraError::Validation(
            format!("Transaction too large: {} bytes (max: {})", data.len(), MAX_TRANSACTION_SIZE)
        ));
    }
    Ok(())
}
...
pub fn validate_did_document(doc: &aura_common::DidDocument) -> Result<()> {
    ...
    if serialized.len() > MAX_DID_DOCUMENT_SIZE {
        return Err(AuraError::Validation(
            format!("DID document too large: {} bytes (max: {})", serialized.len(), MAX_DID_DOCUMENT_SIZE)
        ));
    }
    
    // Validate verification methods
    if doc.verification_method.len() > MAX_ARRAY_LENGTH {
        return Err(AuraError::Validation("Too many verification methods".to_string()));
    }
...
pub fn validate_credential_claims(claims: &serde_json::Value) -> Result<()> {
    ...
    if !claims.is_object() {
        return Err(AuraError::Validation("Claims must be an object".to_string()));
    }
    ...
    if serialized.len() > MAX_CREDENTIAL_SIZE {
        return Err(AuraError::Validation(
            format!("Claims too large: {} bytes (max: {})", serialized.len(), MAX_CREDENTIAL_SIZE)
        ));
    }
    ...
    match value {
        ...
        serde_json::Value::Object(map) => {
            if map.len() > MAX_ARRAY_LENGTH {
                return Err(AuraError::Validation("Object has too many properties".to_string()));
            }
            for (key, val) in map {
