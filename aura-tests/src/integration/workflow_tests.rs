...
use aura_common::{
    ...
    types::BlockNumber,
    vc::{CredentialSchema, Proof, VerifiableCredential},
    AuraDid, Timestamp, TransactionId,
};
use aura_crypto::{
    encryption::{decrypt, decrypt_json, encrypt, encrypt_json, generate_encryption_key},
    keys::{KeyPair, PrivateKey, PublicKey},
    signing::{sign, sign_json, verify, verify_json},
    Signature,
};
use aura_ledger::{
    blockchain::Block,
...
async fn test_complete_did_lifecycle() {
    ...
    // 5. Process transaction
    {
        ...
        registry
            .register_did(&did_doc, public_key.clone(), BlockNumber(1))
            .unwrap();
    }
...
async fn test_complete_vc_workflow() {
    ...
    let _holder_keypair = KeyPair::generate().unwrap();
    ...
    // 4. Register schema
    {
        ...
        registry
            .register_schema(&schema, &issuer_did, BlockNumber(1))
            .unwrap();
    }
    ...
    let cred_id = wallet
        .store_credential(
            credential.clone(),
            vec!["university".to_string(), "degree".to_string()],
        )
        .unwrap();
    ...
    {
        let _registry = revocation_registry.read().await;
    ...
    let revocation_signature =
        sign_json(issuer_keypair.private_key(), &revocation_tx_for_signing).unwrap();

    let _signed_revocation_tx = Transaction {
...
async fn test_wallet_multi_identity_management() {
    ...
    for ((did, _), _) in dids.iter().zip(keypairs.iter()) {
        ...
        wallet
            .store_credential(credential, vec!["identity".to_string()])
            .unwrap();
    }
    ...
    imported_wallet
        .import_wallet(export_data, "test_password")
        .unwrap();
...
async fn test_multi_validator_consensus() {
    ...
    let validators: Vec<KeyPair> = (0..3).map(|_| KeyPair::generate().unwrap()).collect();
    ...
    for (i, validator) in validators.iter().enumerate() {
        ...
        for j in 0..3 {
            ...
            let signature = sign_json(validator.private_key(), &tx_for_signing).unwrap();
        ...
        // Produce block
        {
            ...
            let previous_hash = if i == 0 {
                ...
                storage
                    .get_block(&BlockNumber(i as u64))
                    .unwrap()
                    .unwrap()
                    .hash()
            };
...
async fn test_cross_crate_encryption() {
    ...
    let (_did, _did_doc, keypair) = wallet.create_did().unwrap();
...
async fn test_transaction_validation_pipeline() {
    ...
    for (nonce, tx_type) in tx_types.into_iter().enumerate() {
        ...
        if let Ok(is_valid) = verify_json(
            ...
            assert!(
                !is_valid,
                "Signature should not be valid for wrong chain_id"
            )
        }
    }
}
