...
mod tests {
    ...
    #[test]
    fn test_ed25519_rfc8032_known_answer() {
        // RFC 8032 section 7.1, TEST 1. Pins key derivation, signing and
        // verification across the ed25519-dalek 2 -> 3 upgrade.
        let secret = unhex("9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60");
        let expected_public =
        ...
        let private_key = PrivateKey::from_bytes(&secret).unwrap();
        let public_key = private_key.public_key();
        assert_eq!(public_key.to_bytes().to_vec(), expected_public);

        let signature = sign(&private_key, b"").unwrap();
        assert_eq!(signature.to_bytes(), expected_sig.as_slice());
        assert!(verify(&public_key, b"", &signature).unwrap());

        // Any change to the message must fail verification.
        assert!(!verify(&public_key, b"x", &signature).unwrap());
    }
